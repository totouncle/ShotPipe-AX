...
class MetadataExtractor:
    ...
    def _initialize_exiftool(self):
        ...
        # 1. Check for a bundled exiftool executable first
        try:
            # Path relative to this file (metadata.py)
            ...
            if bundled_exiftool_path.is_file():
                ...
                logger.info(f"Found bundled ExifTool at: {EXIFTOOL_PATH}")
        except Exception as e:
            logger.warning(f"Error checking for bundled exiftool: {e}")

        # 2. If not bundled, search the system PATH
        if not EXIFTOOL_PATH:
            logger.debug("Searching for ExifTool executable in system PATH.")
            EXIFTOOL_PATH = shutil.which('exiftool')
        
        if not EXIFTOOL_PATH:
            logger.warning("ExifTool executable not found in vendor directory or system PATH.")
            EXIFTOOL_AVAILABLE = False
            return False

        logger.info(f"Using ExifTool from: {EXIFTOOL_PATH}")

        # Try to initialize ExifToolHelper, then fallback to older interfaces.
        try:
        ...
        try:
            if hasattr(exiftool, 'ExifTool'):
                self.exiftool = exiftool.ExifTool(executable=EXIFTOOL_PATH)
                self.exiftool.start()
                version = self.exiftool.execute('-ver').strip()
                logger.info(f"ExifTool initialized successfully using legacy ExifTool interface (version {version}).")
                return True
        except Exception as e:
            logger.warning(f"Failed to initialize legacy ExifTool interface, will try direct subprocess. Error: {e}")
        ...
        try:
            result = subprocess.run([EXIFTOOL_PATH, '-ver'], capture_output=True, text=True, timeout=5, check=True)
            ...
            logger.error(f"Failed to execute ExifTool directly. ExifTool functionality will be disabled. Error: {e}")

        # If all attempts fail
        logger.error("Could not initialize any ExifTool interface.")
        self.exiftool = None
        EXIFTOOL_AVAILABLE = False
        return False
